...
function buildSteeringPrompt(evidence: string, lookback: number): string {
  return [
    ...
    "",
    "ENCODING RULE — prefer PROCESS HEURISTICS over bare facts:",
    "Encode each lesson as a CONDITIONAL DECISION RULE: 'When X happens / Before doing Y → do Z'.",
    "A rule is directly actionable at decision time; a bare fact is not. Example:",
    "  ✗ fact:      'loadConfig() caches the config in-process'",
    "  ✓ heuristic: 'If config behavior in tests seems stale → call resetConfigCache() before diagnosing anything else'",
    "One lesson = one rule. Ground every rule in concrete evidence from this trajectory.",
    "",
    "Steps:",
...
function buildSignalSteeringPrompt(signals: string[], evidence: string, lookback: number): string {
  ...
  lines.push(
    ...
    "   Encode lessons as conditional decision rules ('When X / Before Y → do Z'), not bare facts.",
